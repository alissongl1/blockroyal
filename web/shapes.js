...
function CanvasState(canvas) {
    ...
    if (myState.dragging) {
      ...
      for (var i = l - 1; i >= 0; i--) {
        if (shapes[i].contains(mx, my)) {
          ...
          console.log("MOVE Quadrado ID " + keyQuadrado);

          var index = quadrados.indexOf(quadrados.find(quadrado => quadrado.i == keyQuadrado));

          quadrado[keyQuadrado + "/i"] = keyQuadrado
          quadrado[keyQuadrado + "/x"] = mouse.x - 10
          quadrado[keyQuadrado + "/y"] = mouse.y - 10
          quadrado[keyQuadrado + "/w"] = quadrados[index]["w"]
          quadrado[keyQuadrado + "/h"] = quadrados[index]["h"]
  ...
  canvas.addEventListener('dblclick', function (e) {
    ...
    console.log("ADD Quadrado ID " + newQuadradoKey);
...
function init() {
  ...
  var ref = firebase.database().ref('quadrados/');

  var showQuadrado = true

  ref.on("child_added", function(snapshot, prevChildKey) {
    var novoQuadrado = snapshot.val();
    quadrados.push(novoQuadrado);
    s.addShape(new Shape(novoQuadrado));
    console.log("ADD Quadrado ID: " + prevChildKey);
  });

  ref.on("child_changed", function(snapshot) {
    var quadradoModificado = snapshot.val();
    console.log("UPDATE Quadrado ID " + quadradoModificado.i);

    var index = quadrados.indexOf(quadrados.find(quadrado => quadrado.i == quadradoModificado["i"]));

    if (quadradoModificado["x"] == quadrados[index]["x"] && quadradoModificado["y"] == quadrados[index]["y"]) { //não se movimentou
      showQuadrado = false
    }else if (s.dragging && s.draggingIdentity == quadradoModificado["i"]) { // se movimentou mas no cliente
      ...
      s.removeShape(quadradoModificado);
      console.log("UPDATE Quadrado index " + index);
      quadrados[index] = quadradoModificado;
      showQuadrado = true;
    }

    if (showQuadrado) {
      quadrados.push(quadradoModificado);
      s.addShape(new Shape(quadradoModificado));
    }
  });

  ref.on("child_removed", function(snapshot) {
    var quadradoDeletado = snapshot.val();
    //console.log("The square id is " + quadradoDeletado.i + " has been deleted");
  });
  
  //ANTES, TODOS OS DADOS VINHAM DO FIREBASE EM QUALQUER OPERAÇÃO. OU SEJA, SE QUALQUER USUÁRIO
  //UTILIZANDO O APP ALTERASSE A POSIÇÃO DE UM QUADRADO, OU CRIASSE UM QUADRADO
